...
// 初始化日志 logger
func LoggerConfiger(ProjectName string, level string) *zap.Logger {
	loggerRoot := utils.ProjectLocation(ProjectName) + "/logs/"

	infofile := loggerRoot + "/info_%Y%m%d.log"
	errfile := loggerRoot + "/error_%Y%m%d.log"
...
func getWriter(loggerFileName string) io.Writer {
	// 生成rotatelogs的Logger 实际生成的文件名 demo.log.YYmmddHH
	// demo.log是指向最新日志的链接
	hook, err := rotatelogs.New(
		loggerFileName, // 没有使用go风格反人类的format格式.%Y%m%d%H
